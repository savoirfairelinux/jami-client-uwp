...
Contact^
ContactListModel::addNewContact(String^ name, String^ ringId, TrustStatus trustStatus, ContactStatus contactStatus)
{
    auto trimmedName = Utils::Trim(name);
    if (contactsList_ && !findContactByName(trimmedName)) {
        Contact^ contact = ref new Contact(m_Owner, trimmedName, ringId, nullptr, 0, contactStatus, trustStatus);
        contactsList_->Append(contact);
        saveContactsToFile();
        DRing::subscribeBuddy(Utils::toString(m_Owner), Utils::toString(ringId), true);
