...
void
RingD::placeCall(String^ uri)
{
    tasks_.add_task([this, uri]() {
        auto contactItem = ConversationItemsViewModel::instance->findItemByUri(uri);
        auto selectedAccountItem = AccountItemsViewModel::instance->_selectedItem;

        auto _accountId = Utils::toString(selectedAccountItem->_id);
        auto _uri = Utils::toString(contactItem->_uri);

        std::string callId;
        if (contactItem->_type == "RING") {
            callId = DRing::placeCall(_accountId, "ring:" + _uri);
        }
        else if (selectedAccountItem->_accountType == "SIP") {
            callId = DRing::placeCall(_accountId, "sip:" + _uri);
        }
        ...
        Utils::Threading::runOnUIThread([this, callId, contactItem]() {
            contactItem->_callId = Utils::toPlatformString(callId);
