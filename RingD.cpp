    ...
    for (; rit != accountList.rend(); ++rit) {
        ...
        if (type == "RING") {
            ...
            if (account) {
                ...
                if (!ringID.empty())
                    AccountsViewModel::instance->addRingAccount(alias, ringID, accountId, deviceId, upnpState);
            }
        }
        else { /* SIP */
            auto alias = accountDetails.find(DRing::Account::ConfProperties::ALIAS)->second;
            ...
            if (account) {
                ...
                AccountsViewModel::instance->addSipAccount(alias, accountId, sipHostname, sipUsername, sipPassword);
            }
...
RingD::registerCallbacks()
{
    ...
    callHandlers = {
                    ...
                    int code)
        {
            ...
            if (item->_contact->_trustStatus != TrustStatus::TRUSTED)
                return;
                ...
                time_t received)
        {
            ...
            ref new DispatchedHandler([=]() {
                ...
                if (auto contactListModel = AccountsViewModel::instance->getContactListModel(std::string(account_id))) {
                    ...
                    RingD::instance->lookUpName(account_id, Utils::toPlatformString(from));
                    ...
                    vcard->setData(parsedPayload.at("VCARD"));
                    vcard->completeReception();
                    contact->_name = Utils::toPlatformString(vcard->getPart("FN"));
