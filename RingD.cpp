...
RingD::updateAccount(String^ accountId)
{
    tasks_.add_task([this, accountId]() {
        ...
        if (userNameAdded) {
            registerName_new(_accountId, "", newUsername);
        }
...
{
    auto username = Utils::toString(nameToRegister);
    if (shouldRegister) {
        shouldRegister = false;
        bool result = DRing::registerName(accountId, "", username);
    }
