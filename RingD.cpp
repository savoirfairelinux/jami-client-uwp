        ...
        if (type == "RING") {
            ...
            if (auto contactListModel = AccountsViewModel::instance->getContactListModel(std::string(accountId))) {
                for (auto& cr: contactRequests) {
                    auto ringId = cr.first;
                    auto timeReceived = cr.second;
    ...
    trustRequestHandlers =
    {
                ...
                time_t received)
        {
            ...
            ref new DispatchedHandler([=]() {
                ...
                if (auto contactListModel = AccountsViewModel::instance->getContactListModel(std::string(account_id))) {
                    ...
                    // The rest here should be called after a NS lookup

                    // No contact found, so add a new contact with the INCOMNG_CONTACT_REQUEST trust status flag
                    contact = contactListModel->addNewContact(fromP, fromP, TrustStatus::INCOMNG_CONTACT_REQUEST);
            ...
            ref new DispatchedHandler([=]() {
                ...
                if (confirmed) {
                    if (auto contactListModel = AccountsViewModel::instance->getContactListModel(std::string(account_id))) {
                        ...
                        if (contact == nullptr)
                            contact = contactListModel->addNewContact(  Utils::toPlatformString(account_id),
            ...
            [&](const std::string& account_id, const std::string& uri, bool banned)
        {
            ...
            ref new DispatchedHandler([=]() {
                ...
                // It's not clear to me how this signal is pertinent to the client currently
            }));
        })
    };
...
RingD::dequeueTasks()
{
    for (int i = 0; i < tasksList_.size(); i++) {
        ...
        case Request::SendTrustRequest:
        {
            auto contactListModel = AccountsViewModel::instance->getContactListModel(task->_accountId_new);
            auto contact = contactListModel->findContactByRingId(Utils::toPlatformString(task->_ringId_new));
...
void
RingD::sendTrustRequest(const std::string& accountId, const std::string& uri, const std::string& payload)
{
    auto task = ref new RingD::Task(Request::SendTrustRequest);
    task->_accountId_new = accountId;
    task->_ringId_new = uri;
