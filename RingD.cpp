...
RingD::subscribeBuddies()
{
    for (auto accountItem : AccountItemsViewModel::instance->_itemsList) {
        if (Utils::hasInternet()) {
            auto contactItemList = accountItem->_contactItemList;
            for (auto contactItem : contactItemList->_contactItems) {
                MSG_("account: " + accountItem->_id + " subscribing to buddy presence for ringID: " + contactItem->_uri);
                subscribeBuddy(Utils::toString(accountItem->_id), Utils::toString(contactItem->_uri), true);
            }
        }
    }
}
