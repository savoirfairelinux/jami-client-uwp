...
void RingClientUWP::RingD::sendAccountTextMessage(String^ message)
{
    ...
    /* conversation */
    if (sentToken) {
        contact->_conversation->addMessage(MSG_FROM_ME, message, std::time(nullptr), false, sentToken);
...
void RingClientUWP::RingD::sendSIPTextMessage(String^ message)
{
    ...
    contact->_conversation->addMessage(MSG_FROM_ME, message, std::time(nullptr), false, 0);
