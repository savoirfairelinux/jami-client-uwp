    ...
    configurationHandlers = {
        ...
        DRing::exportable_callback<DRing::ConfigurationSignal::NameRegistrationEnded>(
            [this](const std::string& accountId, int state, const std::string& name) {
                MSG_SIG_("SIGNAL<NameRegistrationEnded> " + ("(accountId = " + accountId + ", state = " + state + ", name = " + name + ")"));
...
RingD::onRegistrationStateChanged(String^ accountId, String^ state, int detailsCode, String^ detailsStr)
{
    ...
    auto allAccountDetails = std::make_shared<AccountDetailsBlob>();
    Threading::runOnWorkerThread(
        [this, state, accountId, accountDetails, allAccountDetails]() {
            *accountDetails = DRing::getAccountDetails(toString(accountId));
            *allAccountDetails = std::move(getAllAccountDetails());
        })->Completed = ref new AsyncActionCompletedHandler(
            [this, state, accountId, accountDetails, allAccountDetails](IAsyncAction^ asyncInfo, AsyncStatus asyncStatus) {
                Threading::runOnUIThread(
                    [this, state, accountId, accountDetails, allAccountDetails]() {
                        auto _state = toString(state);
                        if (_state == DRing::Account::States::INITIALIZING) {
                            return;
                        }
                        parseAccountDetails(*allAccountDetails.get());
