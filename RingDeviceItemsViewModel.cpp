...
RingDeviceItem^
RingDeviceItemsViewModel::findItem(RingDevice device)
{
    for each (RingDeviceItem^ item in itemsList)
        if (item->_deviceName == Utils::toPlatformString(device.name))
            return item;
...
unsigned int
RingDeviceItemsViewModel::getIndex(RingDevice device)
{
    for (unsigned i = 0; i < itemsList_->Size; i++) {
        if (itemsList_->GetAt(i)->_deviceName == Utils::toPlatformString(device.name))
            return i;
    }
    return 0;
}
