...
SmartPanel::SmartPanelItem_Grid_PointerReleased(Platform::Object^ sender, Windows::UI::Xaml::Input::PointerRoutedEventArgs^ e)
{
    ...
    if (SmartPanelItemsViewModel::instance->_selectedItem != item) {
        ...
        // TODO: don't summon the video or message page if the user is already in it
        /*auto frame = dynamic_cast<Windows::UI::Xaml::Controls::Frame^>(Window::Current->Content);
        auto currentFrame = dynamic_cast<RingClientUWP::MainPage^>(frame->Content)->currentFrame;*/
        if (contact) {
...
SmartPanel::_revokeDeviceButton__Click(Platform::Object^ sender, Windows::UI::Xaml::RoutedEventArgs^ e)
{
    ...
    auto deviceId = static_cast<RingDeviceItem^>(button->DataContext)->_deviceId;
    ...
    create_task(dialog->ShowAsync())
        ...
        try {
            ...
            if (result == ContentDialogResult::Primary) {
                MSG_("revoking device, (id=" + deviceId + ")");
            }
            else {
    ...
    MSG_("invoke revoke password modal - device, (id=" + deviceId + ")");
}


void RingClientUWP::Views::SmartPanel::_deviceName__KeyUp(Platform::Object^ sender, Windows::UI::Xaml::Input::KeyRoutedEventArgs^ e)
{
    auto textBox = static_cast<TextBox^>(sender);
    auto deviceName = static_cast<RingDeviceItem^>(textBox->DataContext)->_deviceName;
    if (e->Key == Windows::System::VirtualKey::Enter) {
        if (textBox->Text == "") {
            textBox->Text = deviceName;
        }
        else {
            MSG_("changing device name, (" + deviceName + " -> " + textBox->Text + ")");
            static_cast<RingDeviceItem^>(textBox->DataContext)->_deviceName = textBox->Text;
        }
...
{
    auto textBox = static_cast<TextBox^>(sender);
    auto item = static_cast<RingDeviceItem^>(textBox->DataContext);
    textBox->Text = item->_deviceName;
    MSG_("_deviceName__LostFocus - resetting original name (" + item->_deviceName + ")");
}
