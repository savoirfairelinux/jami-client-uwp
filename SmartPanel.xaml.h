...
static public ref class TextBlockExtension sealed
{
public:
    String^ GetFormattedText(DependencyObject^ obj)
    {
        return static_cast<String^>(obj->GetValue(FormattedTextProperty));
    };

    void SetFormattedText(DependencyObject^ obj, String^ value)
    {
        obj->SetValue(FormattedTextProperty, value);
    };

private:
    DependencyProperty^ FormattedTextProperty =
                ...
                ref new PropertyChangedCallback([](DependencyObject^ sender, DependencyPropertyChangedEventArgs^ e)
    {
        ...
        auto textBlock = static_cast<TextBlock^>(sender);
        if (textBlock != nullptr) {
            textBlock->Inlines->Clear();
            std::regex url("((((http|https):\/\/)|www)[^\ ]+)", std::regex_constants::icase);
            std::regex word("([^\ ]+)");
            std::sregex_iterator next(text.begin(), text.end(), word);
            ...
            while (next != end) {
                ...
                run->Text = str;
                if (std::regex_match(match.str(), url)) {
                    //std::cout << "URL: " << match.str() << "\n";
                    Hyperlink^ link = ref new Hyperlink();
                    link->NavigateUri = ref new Uri(str);
                    link->Foreground = ref new Windows::UI::Xaml::Media::SolidColorBrush(Windows::UI::Colors::BlueViolet);
                    link->Inlines->Append(run);
                    textBlock->Inlines->Append(link);
                }
                else {
...
private:
	...
	void _accountEnabled__Toggled(Platform::Object^ sender, Windows::UI::Xaml::RoutedEventArgs^ e);
    void _dhtPublicInCalls__Toggled(Platform::Object^ sender, Windows::UI::Xaml::RoutedEventArgs^ e);
    void _autoAnswerToggle__Toggled(Platform::Object^ sender, Windows::UI::Xaml::RoutedEventArgs^ e);
};
}
}
