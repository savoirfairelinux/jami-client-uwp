        ...
        ref new PropertyChangedCallback([](DependencyObject^ sender, DependencyPropertyChangedEventArgs^ e)
{
    ...
    if (paragraph != nullptr) {
        ...
        static std::regex abs_url_regex("((((https?):\/\/))[^\ ]+)", regex_opts);
        static std::regex url_regex("((((https?):\/\/)|www)[^\ ]+)", regex_opts);
        static std::regex youtube_regex("(https?\:\/\/)?(www\.)?(youtube\.com|youtu\.?be)([^\=]+)=([^\ ]+)", regex_opts);
        static std::regex image_regex("[^\ ]+\.(gif|jpg|jpeg|png)", regex_opts);
        static std::regex word_regex("([^\ ]+)", regex_opts);
