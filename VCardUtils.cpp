...
void
VCard::sendChunk(std::map<std::string, std::string> chunk)
{
    MSG_("send VCARD chunk");
    MSG_("sending chunk of " + std::to_string(chunk.size()) + "bytes");
    RingD::instance->sendSIPTextMessageVCF(chunk);
}
...
VCard::decodeBase64ToPNGFile()
{
    ...
    std::string encodedData = ring::base64::encode(decodedData);
