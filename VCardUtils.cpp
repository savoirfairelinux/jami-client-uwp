...
parseContactRequestPayload(const std::string& payload)
{
    ...
    while (std::getline(_payload, _line)) {
        auto vcardPos = _line.find(Symbols::END_TOKEN);
        if (vcardPos != std::string::npos) {
            map["MESSAGE"].append(_line);
            break;
        }
        map["VCARD"].append(_line.substr(vcardPos));
    }
...
{
    std::stringstream _payload(m_data);
    std::string _line;
