...
VideoPage::OnsmartInfoUpdated(const std::map<std::string, std::string>& info)
{
    ...
    if (smartInfo->HasKey("local FPS"))
        _si_fps1_->Text = "Framerate: " + smartInfo->Lookup("local FPS");
    if (smartInfo->HasKey("local video codec"))
        _si_vc1_->Text = "Video codec: " + smartInfo->Lookup("local video codec");
    if (smartInfo->HasKey("local audio codec"))
        _si_ac1_->Text = "Audio codec: " + smartInfo->Lookup("local audio codec");

    auto localResolution = VideoManager::instance->captureManager()->activeDevice->currentResolution();
    _si_res1_->Text = "Resolution: " + localResolution->getFriendlyName();

    if (smartInfo->HasKey("remote FPS"))
        _si_fps2_->Text = "Framerate: " +  smartInfo->Lookup("remote FPS");
    if (smartInfo->HasKey("remote video codec"))
        _si_vc2_->Text = "Video codec: " +smartInfo->Lookup("remote video codec");
    if (smartInfo->HasKey("remote audio codec"))
        _si_ac2_->Text = "Audio codec: " +smartInfo->Lookup("remote audio codec");

    if (currentRendererWrapper) {
        auto remoteResolution = currentRendererWrapper->renderer->width.ToString() +
                                "x"+
                                currentRendererWrapper->renderer->height.ToString();
        _si_res2_->Text = "Resolution: " + remoteResolution;
    }
}
...
void RingClientUWP::Views::VideoPage::updatePageContent()
{
    ...
    //_contactAvatar_->ImageSource = RingClientUWP::ResourceMananger::instance->imageFromRelativePath(contact->_avatarImage);
    auto avatarImageUri = ref new Windows::Foundation::Uri(contact->_avatarImage);
    _contactAvatar_->ImageSource = ref new BitmapImage(avatarImageUri);
...
void
VideoPage::_videoContent__KeyDown(Platform::Object^ sender, Windows::UI::Xaml::Input::KeyRoutedEventArgs^ e)
{
    MSG_("VIDEOKey");
