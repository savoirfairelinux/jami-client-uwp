    ...
    if (SUCCEEDED(reinterpret_cast<IUnknown*>(reference)->QueryInterface(IID_PPV_ARGS(&byteAccess)))) {
        ...
        for (int row = 0; row < desc.Height; row++) {
            for (int col = 0; col < desc.Width; col++) {
                ...
                auto r = static_cast<double>(buf[currPixel + 0]) / 255.;
                auto g = static_cast<double>(buf[currPixel + 1]) / 255.;
                auto b = static_cast<double>(buf[currPixel + 2]) / 255.;

                auto mag = sqrtf(r * r + g * g + b * b);
                auto val = mag >= 0.4 ? mag >= 0.8 ? 255 : 128 : 0;

                data[currPixel + 0] = val;
                data[currPixel + 1] = val;
                data[currPixel + 2] = val;
            }
        }
    }
    delete reference;
