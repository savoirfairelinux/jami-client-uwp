...
void RingClientUWP::Views::VideoPage::PreviewImage_ManipulationDelta(Platform::Object^ sender, ManipulationDeltaRoutedEventArgs^ e)
{
    ...
    GeneralTransform^ transform = PreviewImageRect->TransformToVisual(container);
...
void RingClientUWP::Views::VideoPage::PreviewImage_ManipulationCompleted(Platform::Object^ sender, ManipulationCompletedRoutedEventArgs^ e)
{
    ...
    UIElement^ container = dynamic_cast<UIElement^>(VisualTreeHelper::GetParent(_videoContent_));
    GeneralTransform^ transform = PreviewImageRect->TransformToVisual(container);
    Point relativePreviewTL = transform->TransformPoint(Point(0, 0));
    //Point relativePreviewTL = Point(Canvas::GetLeft(PreviewImageRect), Canvas::GetTop(PreviewImageRect));

    Point targetCornerPoint;
    switch (quadrant)
    {
    case 0:
        targetCornerPoint = Point(  static_cast<float>(_videoContent_->ActualWidth) - scaledWidth,
                                    static_cast<float>(_videoContent_->ActualHeight) - scaledHeight    );
        break;
    case 1:
        targetCornerPoint = Point(  0,
                                    static_cast<float>(_videoContent_->ActualHeight) - scaledHeight    );
        break;
    case 2:
        targetCornerPoint = Point(  0,
    ...
    MSG_("targetCornerPoint: " + targetCornerPoint.X.ToString() + " , " + targetCornerPoint.Y.ToString());
    MSG_("relativePreviewTL: " + relativePreviewTL.X.ToString() + " , " + relativePreviewTL.Y.ToString());
    MSG_("scaleValue: " + scaleValue.ToString());

    PreviewImage_deltaTransform->TranslateX = targetCornerPoint.X - relativePreviewTL.X;
    PreviewImage_deltaTransform->TranslateY = targetCornerPoint.Y - relativePreviewTL.Y;
}
